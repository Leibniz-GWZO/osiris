...
class OpenAlexParser:
    def __init__(self, ignore_duplicates=False) -> None:
        ...
        if not ignore_duplicates:
            ...
            self.possible_dupl = [(i["_id"], i["title"]) for i in possible_dupl]
    ...
    def getJournal(self, issn):
        #1 Preliminary check: ISSN list available and not empty?
        if not issn or not isinstance(issn, list):
            return None
        ...
        # 3. if not found: OpenAlex query
        # We assume that .get_single_venue(issn[-1], "issn") 
        # uses the last ISSN entry:
        try:
            source = self.openalex.get_single_venue(issn[-1], "issn")
        except requests.exceptions.HTTPError as e:
            # 404 or other HTTP error -> we return None
            print(f"OpenAlex returned an error for ISSN {issn[-1]}: {e}")
            return None

        # 4. check whether the response is usable at all
        if not source or source.get("type") != "journal":
            return None
    ...
    def parseWork(self, work):
            ...
            authors.append(
                {
                    ...
                    "position": pos,
                    "aoi": (
                        "https://openalex.org/" + self.inst_id
                        in [i.get("id") for i in a["institutions"]]
                    ),
                    "orcid": orcid,
                    "user": user,
                    "approved": False,
        ...
        if (typ == 'article'):
            if not loc or not loc['issn']:
                ...
                if journal is None:
                    # Case1 No Journal -> fallback
                    element['subtype'] = 'magazine'
                    element['magazine'] = loc.get('display_name') if loc else None
                else:
                    # Case2 Journal exists
                    element.update({
